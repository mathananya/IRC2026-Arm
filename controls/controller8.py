...
class AckermannDriveController(Node):
    def __init__(self):
        ...
        self.get_logger().info("Drive Controller Started. Modes: [h] Home, [n] Crab, [c] Pivot. Actuator: [1]/[2]. Streaming at 20Hz...")
    ...
    def keyboard_callback(self, msg):
        ...
        if key == '1':
            ...
            return  # Skip the rest of the loop so it doesn't mess with drive filters
        elif key == '2':
    ...
    def publish_state(self):
        ...
        now = self.get_clock().now()
        if (now - self.last_actuator_time).nanoseconds / 1e9 > 0.2:
            self.actuator_cmd = 0.0  # Stop actuator if key released for > 0.2s
        ...
        if self.mode == 'crab':
            ...
            v_lf, v_lr, v_rf, v_rr = -self.vx, -self.vx, self.vx, self.vx
        ...
        # 3. ACKERMANN MODE (Your Original Math)
            ...
            if abs(self.center_steer) < 0.001:
                ...
                v_of = v_base                  
                v_if = omega * r_steered_inner 
                v_ob = omega * r_fixed_outer   
                v_ib = omega * r_fixed_inner   
                
                if self.center_steer > 0:
                    mag_lf, mag_lr, mag_rf, mag_rr = v_of, v_ob, v_if, v_ib
                else:
                    mag_lf, mag_lr, mag_rf, mag_rr = v_if, v_ib, v_of, v_ob
        ...
        # Append actuator_cmd as the 5th element in the array
        msg_wheels.data = [float(v_lf), float(v_lr), float(v_rf), float(v_rr), float(self.actuator_cmd)]
        self.pub_wheels.publish(msg_wheels)
        
        # Dashboard Feedback (added actuator state)
        print(f"\rMode: {self.mode.upper():<9} | Speed: {self.vx:5.1f} | Input: {self.center_steer:5.1f}° | W0(L): {angles[0]:5.1f}° | W2(R): {angles[2]:5.1f}° | Act: {int(self.actuator_cmd)}    ", end="", flush=True)
