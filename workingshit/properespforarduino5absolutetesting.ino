...
const int ABS_ENC_PIN[4] = { 19, 14, 26, 33 };
int target_angle[4] = { 0, 1365, 0, 1650 };
...
void subscription_callback(const void * msgin) {
  ...
  if (msg->data.size >= 4) {
    ...
    angles[1] += 120.0; // Wheel 1 offset
    angles[3] += 145.0; // Wheel 3 offset
...
void loop() { 
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (abs(error[i]) <= TOLERANCE) {
      ...
      setMotor(MOTOR_SPEED, (i == 3) ? false : true, pwm[i], dir[i]);
    } else {
      digitalWrite(feedback_pin, HIGH);
      // Target is behind -> Backward
      setMotor(MOTOR_SPEED, (i == 3) ? true : false, pwm[i], dir[i]);
    }
